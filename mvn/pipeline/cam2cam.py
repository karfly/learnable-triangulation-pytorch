...
def batch_iter(batch, iter_i, model, cam2cam_model, criterion, opt, scheduler, images_batch, keypoints_3d_gt, keypoints_3d_binary_validity_gt, is_train, config, minimon):
    ...
    def _forward_cam2cam(pairs, gts=None):
    ...
    def _do_dlt(cam2cam_preds, keypoints_2d_pred, master_cam_i=0):
        ...
        for batch_i in range(batch_size):
            master_cam = batch['cameras'][master_cam_i][batch_i]
            target_cams_i = [1, 2, 3]  # todo use `master_cam`
            target_cams = [
                batch['cameras'][cam_i][batch_i]
        ...
        return torch.cat([
            batch['cameras'][master_cam_i][batch_i].cam2world()(
                keypoints_3d_pred[batch_i]
            ).unsqueeze(0)
            for batch_i in range(batch_size)
        ])

    def _compute_losses(cam2cam_preds, cam2cam_gts):
        _pairs = [
            [0, 1],
            [0, 2],
            [0, 3]
        ...
        pose_loss = twod_proj_loss(
            keypoints_3d_gt, keypoints_3d_pred, batch['cameras']
        )
    ...
    minimon.enter()
    cam2cam_preds = _forward_cam2cam(pairs)
    ...
    minimon.enter()
    keypoints_3d_pred = _do_dlt(cam2cam_preds, keypoints_2d_pred)
